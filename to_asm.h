...
class ToAsm{
	...
	static instruction and_(three_items ti){
		//(&&, var/i/d, var/i/d, var)
		instruction result;
		unsigned judge_result[2];
		bool is_real = judge_and_change_to_real(ti, judge_result);
	...
	static instruction or_(three_items ti){
		//(||, var/i/d, var/i/d, var)
		instruction result;
		unsigned judge_result[2];
		bool is_real = judge_and_change_to_real(ti, judge_result);
	...
	static instruction not_(three_items ti){
		//(!, var/i/d, _, var)
		unsigned judge_result[] = { judge(ti[0]), judge(ti[2]) };
		instruction result;
		if (judge_result[0] == ABSTRACT_VAR){
...
public:
	static void set_masm32_file(){
		std::cout << "Please input the masm32 install directory (e.g: C:\\masm32) " << std::endl;
	...
	static void call_masm(std::string filename){
		...
		full_path = Wmasm32_directory + L"\\ml.exe /c /coff " + filename + L".asm";
		lstrcpyW(new_full_path, full_path.c_str());
#else
		full_path = masm32_directory + "\\bin\\ml.exe /c /coff " + filename + ".asm";
		...
		lstrcpyW(new_full_path, full_path.c_str());
#else
		full_path = masm32_directory + "\\bin\\link.exe /subsystem:console " + filename + ".obj";
		strcpy(new_full_path, full_path.c_str());
#endif
		ZeroMemory(&si, sizeof(STARTUPINFO));
		si.dwFlags = STARTF_USESHOWWINDOW;
		...
		lstrcpyW(new_full_path, full_path.c_str());
#else
		full_path = std::string(now_directory) + "\\" + filename + ".exe";
		strcpy(new_full_path, full_path.c_str());
#endif
...
std::vector<instruction(*) (three_items)> ToAsm::opvs = {
	...
	above_equal, below_equal, equal, not_equal, and_, or_,
	not_, print, scan, end
};
